    ...
    <>
      {aboutDetail?.length > 0 &&
        aboutDetail.map((item, i) => {
          const isLast = i === aboutDetail.length - 1;
          return (
            <div key={i}>
              ...
              <div
                className={`hidden lg:grid md:grid-cols-12 ${
                  isLast ? "items-start" : ""
                }`}
              >
                <div className="md:col-span-6 lg:col-span-4 h-auto flex justify-center items-start">
                  {i % 2 === 0 ? (
                ...
                <div className="md:col-span-6 lg:col-span-8 h-auto flex justify-around items-start">
                  {i % 2 === 0 ? (
                    <div className="w-full relative md:h-full lg:min-h-[327px]">
                      <ImageContent url={item.image} />
                    </div>
...
function TextContent({ title, text }) {
  return (
    <div className="mb-[20px] md:mb-[40px] lg:mb-0 px-20 lg:px-0 text-[var(--color-gray-900)] lg:py-[3rem] xl:py-[5rem]">
      <h3 className="text-base mt-[18px] md:mt-[36px] lg:mt-0 title text-[var(--color-gray-900)] font-[500]">
        {title}
      </h3>
      <p className="text-[1rem] md:text-base mt-[15px]   md:overflow-y-auto">
        {text}
      </p>
    </div>
  );
